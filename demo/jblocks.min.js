...
a,this.applyLayout())):(this.layoutSign=a,this.rebuild())};c.prototype.checkLayout=function(){if(E){var a=this;this.jBlocksLayoutTimer&&clearTimeout(this.jBlocksLayoutTimer);this.jBlocksLayoutTimer=setTimeout(function(){this.jBlocksLayoutTimer=null;a.doCheckLayout()},this.options.layoutDelay)}};c.prototype.makeScrollable=function(){function a(b,d){var e,j,f,g,h,l;2>arguments.length&&(d=1);switch(b){case "down":e="r";j="h";f="top";g="uHeight";h="rows";l=1;break;case "up":e="r";j="h";f="top";g="uHeight";
h="rows";l=-1;break;case "right":e="c";j="w";f="left";g="uWidth";h="cols";l=1;break;case "left":e="c",j="w",f="left",g="uWidth",h="cols",l=-1}for(var m=Math.ceil(d/c.geometry[g]),x=c.viewport[e];Math.abs(x-c.viewport[e])<m;){var k=-1,J;for(J in c.layoutData.rmk[h]){var p=c.layoutData.rmk[h][J];if((0<l&&p>c.viewport[e]||0>l&&p<c.viewport[e])&&(0>l||p+c.viewport[j]<=c.layoutData.maxSize[j])&&(0>k||0<l&&p<k||0>l&&p>k))k=p}if(0<=k)c.viewport[e]=k,p={},p[f]=-k*c.geometry[g],c.frameContent.stop().animate(p,
c.options.scrollAnimateDelay);else break}}if(E){var c=this;this.anchor.bind("mousewheel DOMMouseScroll",function(b){var d;if(b.detail)d=0>b.detail?1:-1;else if(b.wheelDelta)d=0<b.wheelDelta?1:-1;else if(b.originalEvent)if(b.originalEvent.wheelDelta)d=0<b.originalEvent.wheelDelta?1:-1;else if(b.originalEvent.detail)d=0>b.originalEvent.detail?1:-1;else return;else return;b.ctrlKey||("width"==c.viewport.fixed?0>d?a("down"):a("up"):"height"==c.viewport.fixed&&(0>d?a("right"):a("left")));b.stopPropagation()});
var d,e;this.anchor.swipe({swipeLeft:function(b,c,d){a("right",d);b.stopPropagation()},swipeRight:function(b,c,d){a("left",d);b.stopPropagation()},swipeUp:function(b,c,d){a("down",d);b.stopPropagation()},swipeDown:function(b,c,d){a("up",d);b.stopPropagation()},swipeStatus:function(a,j,f,g){switch(j){case "start":d=parseInt(c.frameContent.css("top"));e=parseInt(c.frameContent.css("left"));break;case "cancel":c.frameContent.css({top:d,left:e});break;case "move":"width"==c.viewport.fixed?"down"==f?c.frameContent.css("top",
Math.min(0,d+g)):"up"==f&&c.frameContent.css("top",Math.min(d-g,c.anchor.height()-c.frameContent.height())):"height"==c.viewport.fixed&&("left"==f?(b("head title").text(e+g+" "+c.anchor.width()+" "+c.frameContent.width()),c.frameContent.css("left",Math.max(e-g,c.anchor.width()-c.layoutData.maxSize.w*c.geometry.uWidth))):"right"==f&&c.frameContent.css("left",Math.min(0,e+g)))}}});this.scrollable=!0}};c.prototype.unmakeScrollable=function(){this.anchor.swipe("destroy");this.anchor.unbind("mousewheel DOMMouseScroll")};
c.prototype.updateViewportFromGeometry=function(a,c,d){var e={pixels:150,minw:6,maxw:15,minh:7,maxh:12};2<arguments.length&&b.extend(e,d);c.w=Math.round(a.width/e.pixels);c.w<e.minw&&(c.w=e.minw);c.w>e.maxw&&(c.w=e.maxw);c.h=Math.round(a.height/e.pixels);c.h<e.minh&&(c.h=e.minh);c.h>e.maxh&&(c.h=e.maxh)};c.prototype.getFullscreenMethods=function(){var a=["","moz","webkit","o","ms"],b=null,c;for(c in a){var d=a[c],e=(d+"RequestFullScreen").substr(0,1).toLowerCase()+(d+"RequestFullScreen").substr(1);
if("function"==typeof this.anchor[0][e]){b={request:e,event:d+"fullscreenchange",element:(d+"FullScreenElement").substr(0,1).toLowerCase()+(d+"FullScreenElement").substr(1)};switch(d){case "webkit":b.element="webkitCurrentFullScreenElement"}break}}return b};c.prototype.pushLayout=function(){var a={},b;for(b in this.blocks)a[this.blocks[b].id]=JSON.parse(JSON.stringify(this.blocks[b].layout));this.layoutsStack.push(a)};c.prototype.popLayout=function(){var a=this.layoutsStack.pop(),b;for(b in a){var c=
a[b],d=this.blocks[b];d&&(d.layout=c)}};c.prototype.hideAllBlocks=function(){for(var a in this.blocks)this.blocks[a].layout.visible=!1};c.prototype.updateSize=function(){this.anchor.trigger("jBlocks.resize",{width:this.anchor.width(),height:this.anchor.height()})};c.prototype.canFullscreen=function(){return null!=this.fullscreenMethods};c.prototype.fullscreen=function(a,c){if(0==arguments.length||!a)a=this.anchor;2>arguments.length&&(c=function(){});var d=this.fullscreenMethods;if(d){var e=document[d.element],
j=a.width(),f=a.height();b(document).bind(d.event,function(){setTimeout(function(){var g,h,l;document[d.element]?(l=!0,a.addClass("jblocks-full-size"),g=document[d.element].offsetWidth,h=document[d.element].offsetHeight,b.jBlocks("log","entered fullScreen",g,"x",h)):(l=!1,a.removeClass("jblocks-full-size"),g=j,h=f,b(document).unbind(d.event),b.jBlocks("log","exited fullScreen",g,"x",h));e&&(e.width(g),e.height(h));c(l)},0)});a[0][d.request]()}};var V=[],M=[],E=!1;b(document).ready(function(){if(!E){E=
!0;for(var a in m)b.jBlocks("log",m[a]);m=[];for(a in V)V[a].init();V=null;for(a in M){var c=M[a];c[0].apply(c[1],c[2])}M=null}});var L=null,I,A,P=null,R=null,U={};b.jBlocks=function(a){switch(a){case "defineBlockClass":return v.apply(null,arguments);case "autoSyncContentSize":var d=function(){b("body").trigger("jBlocks.resize",{width:b("body").width(),height:b("body").height()})};b(window).bind("orientationchange resize",function(){d()});b(document).ready(function(){d()});break;case "log":var f=
Array.prototype.slice.call(arguments,1);t.apply(n(),[(new Date).getTime()-j].concat(f));break;case "addTranslation":return f=Array.prototype.slice.call(arguments,1),u.apply(null,f);case "translate":return f=Array.prototype.slice.call(arguments,1),k.apply(null,f);case "getFrameData":return Array.prototype.slice.call(arguments,1),r.call(null);case "addSound":f=Array.prototype.slice.call(arguments,1);E?e.apply(null,f):M.push([e,null,f]);break;case "playSound":f=Array.prototype.slice.call(arguments,1);
E?l.apply(null,f):M.push([l,null,f]);break;case "click":return P||(P="click","ontouchstart"in document.documentElement&&(P="jblocks.click")),P;case "getBlockAnonIndex":return q++;default:if("function"!=typeof c.prototype[a])throw"jBlocks: no such method "+a;var g=n(),f=Array.prototype.slice.call(arguments,1);return c.prototype[a].apply(g,f)}};b.fn.jBlocks=function(){var a=this.first(),b=new c(a);b.anchor=a;b.init();b.updateSize();return b};if("ontouchstart"in document.documentElement){var S=0,K;b(document).bind("touchstart",
function(a){S=(new Date).getTime();K=a.target});b(document).bind("touchend",function(){S&&(200>(new Date).getTime()-S&&b(K).trigger("jblocks.click"),K=null)});b(document).bind("touchcancel",function(){S=0;K=null})}})(jQuery);
